...
# 保存先指定
f = open('Identification_omni_3_11.csv', 'ab')   # wb　write only(binaly file) ab append(binaly file)

dataWriter = csv.writer(f)
# 速度指令部分
Velocity_reference=np.array([[0.3],[0],[0]])
K = 1           #P制御のパラメータ決定
##State_reference=np.array([[1.5],[0],[0]])
oldtheta=0
# flag管理部分
flag_orange=1
flag_green=1
...
##    for i in range(0,4):
##        if duty_rpm[i][0]>dutyMAX:
##            duty_rpm[i][0]=dutyMAX
...
##        if oldtheta==0:
##            oldtheta=State_purple[2][0]
##            State_reference[2][0]=oldtheta
...
##            Sigma_State_purple[0][0]=State_reference[0][0]-State_purple[0][0]
##            Sigma_State_purple[1][0]=0
##            Sigma_State_purple=State_blue-State_purple
##            Sigma_State_purple=Coordinate_tf(Sigma_State_purple,-State_purple[2][0])
##            duty1,direction1=calculation_duty(K*Sigma_State_purple)
...
##            a=np.array([out_time,out_Statex,out_Statey,out_Statetheta])
